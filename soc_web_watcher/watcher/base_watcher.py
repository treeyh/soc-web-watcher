...
class BaseWatcher(object):
  ...
  def send_wx_msg(self, id, msg):
    ...
    if self.check_msg_send(id):
      ...
      self._logger.info('send msg over msgid:%s, users:%s, msg: %s' %
                        (id, str_utils.json_encode(self._send_wx_users), msg))
      return True
    return False
  ...
  def send_ding_msg(self, id, msg):
    ...
    # self._logger.info('send msg: %s' % msg)
    if self.check_msg_send(id):
      result = requests.post(config.APP_CONFIG['dingding_send_url'], data=msg.encode(
          'utf-8'), headers=self._ding_headers)
      if config.APP_CONFIG['is_print_detail']:
        self._logger.info('send ding msg over msgid:%s, state: %d, result:%s, msg: %s' %
                          (id, result.status_code, result.text, msg))
      return True
    return False
